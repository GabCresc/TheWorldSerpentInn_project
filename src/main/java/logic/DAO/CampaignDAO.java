...
public class CampaignDAO {
    ...
    public List<Model_Campaign> retrieveMyCampaigns() throws SQLException{
        List<Model_Campaign> list = new ArrayList<>();

        try(Connection conn = SingletonDBSession.getInstance().startConnection()){
            Statement statement = conn.createStatement();
            ResultSet rs = statement.executeQuery("SELECT * FROM campaign"); // eventualmente tramite una JOIN recuperare nomi DM e conteggio player
                while(rs.next()){
                    ...
                    camp.setCampMode(rs.getString("mode"));
                    camp.setCampDay(rs.getString("day"));
                    camp.setCampCity(rs.getString("city"), rs.getString("day"));
...
// metodo per registrare la richiesta del player nel database
    ...
    public List<Integer> retrieveListofWaitingPlayers (int campaignID) throws SQLException {
